...
#################################################
# initial w
Wmap = np.random.rand(784+1, hidden_layer_num) * 0.01
Wnew = np.random.rand(hidden_layer_num+1, 2)* 0.01
Wkm = np.random.rand(2+1, 10)* 0.01
y = np.zeros((12000,10))
...
for epoch in range(0,epochs):
    ...
    for batch in range(0, 12000, mini_batch):
        ...
        hidden1 = np.matmul(hidden_bias, Wnew)
        hidden1_bias = np.append(hidden1, np.ones((mini_batch, 1)), axis=1)
        hidden2 = np.matmul(hidden1_bias, Wkm) #mini_batch*10
        ...
        de_error2_Wkm = np.matmul(de_error2_hidden2.transpose(), hidden1_bias) #10*1025
        de_error2_hidden1 = np.matmul(de_error2_hidden2, Wkm.transpose()) #m*3
        de_error2_Wnew = np.matmul(de_error2_hidden1.transpose(), hidden_bias)  # m*10
        de_error2_hidden = np.matmul(de_error2_hidden1[:,0:2], Wnew.transpose())  # mini_batch*1025
        ...
        Wnew = np.subtract(Wnew, np.multiply(de_error2_Wnew[0:2, :].transpose(), eta))
    ...
    error.append(np.sum(error_matrix))#+ regulation)
    ...
    a111 = np.matmul(a11_bias, Wnew)
    a1111_bias = np.append(a111, np.ones((5768, 1)), axis=1)
    a2 = np.matmul(a1111_bias, Wkm) #mini_batch*10
    ...
    if epoch == 10:
        ...
        for i in range (0,5768):
            if np.argmax(y_test[i,:])==0:
                ax.scatter(a111[i,0],a111[i,1], alpha=0.8, c="red", edgecolors='none', s=30, label="0")
            elif np.argmax(y_test[i,:])==1:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="green", edgecolors='none', s=30, label="1")
            elif np.argmax(y_test[i,:])==2:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="blue", edgecolors='none', s=30, label="2")
            elif np.argmax(y_test[i,:])==3:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="orange", edgecolors='none', s=30, label="3")
            elif np.argmax(y_test[i,:])==4:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="yellow", edgecolors='none', s=30, label="4")
            elif np.argmax(y_test[i,:])==5:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="purple", edgecolors='none', s=30, label="5")
            elif np.argmax(y_test[i,:])==6:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="tan", edgecolors='none', s=30, label="6")
            elif np.argmax(y_test[i,:])==7:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="lightblue", edgecolors='none', s=30, label="7")
            elif np.argmax(y_test[i,:])==8:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="cyan", edgecolors='none', s=30, label="8")
            elif np.argmax(y_test[i,:])==9:
                ax.scatter(a111[i, 0], a111[i, 1], alpha=0.8, c="brown", edgecolors='none', s=30, label="9")

        plt.title("2D feature " + str(epoch) + " epoch")
        plt.savefig("2D feature " + str(epoch) + " epoch.png")
        plt.show()
        plt.close()
