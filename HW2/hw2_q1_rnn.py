...
with open('covid_19.csv', newline='') as csvfile:
    ...
    hmap.figure.savefig("1Correlation.png",format = 'png',dpi = 300)
    ...
    hmap1.figure.savefig("1Correlation1.png", format='png', dpi=300)
...
#t_x = x_train.reshape((len(x_train), window_size))
t_y = y_train.reshape((len(y_train), 2))
'''#######################################################
t_x = t_x.transpose()
t_x = t_x.reshape(window_size, len(x_train), 1)
t_y = t_y.transpose()
t_y = t_y.reshape((2,len(y_train)))
######################################################'''
...
'''########################################################
tt_x = tt_x.transpose()
tt_x = tt_x.reshape(window_size, len(x_test), 1)
tt_y = tt_y.transpose()
tt_y = tt_y.reshape((2,len(y_test)))
#######################################################'''
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if is_cuda:
            self.rnn.cuda()'''
        out, hidden = self.rnn(x.float(), hidden.float())
...
for epoch in range(1, n_epochs + 1):
    ...
    for i in range(0,len(x_test),batch_size):
        ...
        input_seq.to(device)
        output = model(input_seq[i:i+batch_size])
        out_t = output.reshape(batch_size, 9, 2)
        # out_t = output.reshape(9, 7881, 2)
        out_t_last = out_t[:, -1, :]
        # out_t_last = out_t[-1, : , :]
        ...
        # print(prob.size())
        # print(max.size())
        # loss = criterion(out_t_last.reshape((7881,2)), target_seq)
        loss = criterion(out_t_last.reshape((batch_size, 2)), target_seq[i:i+batch_size])
        loss.backward()  # Does backpropagation and calculates gradients
        optimizer.step()  # Updates the weights accordingly
        prediction = max
        val, ind = torch.max(target_seq[i:i+batch_size], 1)
        correct += (prediction == ind).sum().float()
        total += len(target_seq[i:i+batch_size])
    ...
    if epoch % 2 == 0:
        ...
        tout_t = tout.reshape(5254, 9, 2)
        # tout_t = tout.reshape(9, 5254, 2)
...
plt.figure()
plt.plot(plot_loss)
plt.savefig("1loss_train_rnn.png")
plt.figure()
plt.plot(plot_acc)
plt.savefig("1acc_train_rnn.png")
plt.figure()
plt.plot(tplot_acc)
plt.savefig("1acc_test_rnn.png")
...
worldmap_chart.render_to_file('1world_map_rnn.svg')
