...
def get_label():
    label = []
    with open('id2tag.txt') as f:
        for l in f:
            label.append(l.split('\t')[0].split('\n')[0])
    return label

class Seqdata():
    def __init__(self, embedding_size, is_train):
        self._M_train, self._M_test, self._max_len = np.load(os.path.join(os.getcwd(), 'meta.npy'),allow_pickle=True)[()]
...
def get_meta():
    ...
    meta_train_data = []
    with open(_join_path('train_cut_shuffle.txt'), 'r', encoding='utf-8') as f:
        for l in f:
            label = l.split('\t')[0]
