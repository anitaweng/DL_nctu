...
parser.add_argument("--epsilon_decay", default=500000, type=float)
...
class DQN(object):
    ...
    def select_action(self, state):
        ...
        self.epsilon = self.EPS_END + np.maximum(
            (self.EPS_START - self.EPS_END) * (1 - self.interaction_steps / self.EPS_DECAY), 0)
...
def train():
    ...
    for i_episode in range(num_episodes):
        ...
        for _ in range(10000):
            ...
            if global_steps > 50000:  # 50000
                agent.update()

            if done:
                reward_array.append(episode_reward)
                train_info_str = "Episode: %6d, interaction_steps: %6d, reward: %2d, epsilon: %f" % (
                i_episode, agent.interaction_steps, episode_reward, agent.epsilon)
                print(train_info_str)
                log_fd.write(train_info_str)
                reward_nparray = np.array(reward_array)
                np.save('./model/reward.npy', reward_nparray)
                break
        ...
        if i_episode % args.eval_per_ep == 0:
            ...
            eval_reward_lst.append(average_reward)
            eval_info_str = "Evaluation: True, Episode: %6d, Interaction_steps: %6d, evaluate reward: %f" % (
            i_episode, agent.interaction_steps, average_reward)
            print(eval_info_str)
            log_fd.write(eval_info_str)
            np.save('./model/eval_reward', np.array(eval_reward_lst))
    ...
    for i_episode in range(10):
        ...
        for _ in range(10000):
            ...
            if done:
                ...
                i_episode, agent.interaction_steps, episode_reward, 0.1))
                break
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        model_path = "./lr1e4/q_target_checkpoint_1538048.pth"
        test(model_path)


reward_lst = list(np.load('./lr1e4/reward.npy'))
plt.plot(reward_lst)
plt.title('DQN total rewards for each episode (gamma=0.8)')
plt.xlabel('episode')
plt.ylabel('reward')
plt.savefig('./lr1e4/reward.png')
plt.show()

eval_reward_lst = list(np.load('./lr1e4/reward_eval.npy'))
reward_x = list(np.arange(len(eval_reward_lst))*10)
#print(len(eval_reward_lst))
plt.plot(reward_x, eval_reward_lst)
plt.title('DQN eval rewards for each episode (gamma=0.8)')
plt.xlabel('episode')
plt.ylabel('reward')
plt.savefig('./lr1e4/eval_reward.png')
plt.show()
...
class LearntAgent(object):
    ...
    def evaluate_action(self, state, rand=0.1):
        if random.random() < rand:
            return torch.tensor([[random.randrange(self.action_dim)]], device=device, dtype=torch.long), np.array(
                [[0., 0., 0., ]])
        with torch.no_grad():
...
def play(path):
    ...
    for i_episode in range(1):
        ...
        for s in range(10000):
            ...
            plt.tight_layout()
            plt.savefig('./lr1e4/pic/{}.png'.format(s + 1))
